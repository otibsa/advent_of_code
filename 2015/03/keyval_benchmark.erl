...
bench(Reps) ->
    io:format("Base Case: [], null/3 ...~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(base_case(Reps)),
    
    io:format("~nTuple list: [], ...~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(tuple_list(Reps)),

    io:format("~nNaive Orddict: new/0, store/3, find/2, store/3 erase/2~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(naive_orddict(Reps)),
    
    io:format("~nSmart Orddict: new/0, store/3, fetch/2, store/3, erase/2~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(smart_orddict(Reps)),
    
    io:format("~nNaive Dict: new/0, store/3, find/2, store/3, erase/2~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(naive_dict(Reps)),
    
    io:format("~nSmart Dict: new/0, store/3, fetch/2, store/3, erase/2~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(smart_dict(Reps)),
    
    io:format("~nNaive gb_trees: empty/0, enter/3, lookup/2, enter/3, delete_any/2~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
    print(naive_gb_trees(Reps)),
    
    io:format("~nSmart gb_trees: empty/0, insert/3, get/2, update/3, delete/2~n"),
    io:format("Operation\tTotal (µs)\tAverage (µs)~n"),
...
tuple_list(Reps) ->
    benchmark(Reps,
        ...
        fun(Key, List) -> lists:keysearch(Key, 1, List) end,
        fun(Key, NewValue, List) -> lists:keyreplace(Key, 1, List, {Key, NewValue}) end,
        fun(Key, List) -> lists:keytake(Key, 1, List) end).
